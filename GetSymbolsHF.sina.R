GetSymbolsHF.sina<- function(code, date, market,
  ...
  link <- paste("http://market.finance.sina.com.cn/downxls.php?date=",
                substr(date, 1, 4), "-", substr(date, 5, 6), "-",
                substr(date, 7, 8),
                "&symbol=", tolower(substr(codename, 1, 8)),
                sep="")
